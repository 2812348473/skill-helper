...
export function archive(bytes, { name, subdirectory } = {}) {
  try { return readArchive(Buffer.from(bytes), name, subdirectory) } catch (error) {
    if (error.code && !error.code.startsWith('ERR_') && !error.code.startsWith('Z_')) throw error
    throw fail('INVALID_ZIP')
  }
}
function readArchive(data, name, selected) {
  ...
  if (data.readUInt16LE(end + 4) || data.readUInt16LE(end + 6) || data.readUInt16LE(end + 8) !== count ||
    count > LIMITS.entries || offset + size !== end) throw fail('ZIP_LAYOUT_UNSUPPORTED')
  const entries = [], spans = [], seen = new Set()
  ...
  for (let i = 0; i < count; i++) {
    ...
    const crc = data.readUInt32LE(cursor + 16), compressed = data.readUInt32LE(cursor + 20), original = data.readUInt32LE(cursor + 24)
    const length = data.readUInt16LE(cursor + 28), extra = data.readUInt16LE(cursor + 30), comment = data.readUInt16LE(cursor + 32)
    const mode = (data.readUInt32LE(cursor + 38) >>> 16) & 0xf000, local = data.readUInt32LE(cursor + 42)
    const rawName = data.subarray(cursor + 46, cursor + 46 + length)
    const path = rawName.toString('utf8'), directory = path.endsWith('/')
    const clean = directory ? path.slice(0, -1) : path
    if (Buffer.from(path).compare(rawName) !== 0 || clean.split('/').length > 7) throw fail('INVALID_FILE_PATH')
    clean.split('/').forEach(segment)
    if (seen.has(clean.toLowerCase())) throw fail('DUPLICATE_FILE')
    seen.add(clean.toLowerCase())
    if ((flags & 1) || (flags & 0x40) || ![0, 8].includes(method) || ![0, 0x4000, 0x8000].includes(mode) ||
      data.readUInt16LE(cursor + 34) || (directory && original !== 0) ||
      (mode === 0x4000 && !directory) || (mode === 0x8000 && directory)) throw fail('ZIP_SPECIAL_ENTRY')
    total += original
    if (original > LIMITS.file || total > LIMITS.total) throw fail('SIZE_LIMIT')
    if (data.readUInt32LE(local) !== 0x04034b50 || local >= offset ||
      data.readUInt16LE(local + 6) !== flags || data.readUInt16LE(local + 8) !== method) throw fail('INVALID_ZIP')
    const localLength = data.readUInt16LE(local + 26), localExtra = data.readUInt16LE(local + 28)
  ...
  if (cursor !== end) throw fail('INVALID_ZIP')
  const candidates = entries.filter((e) => !e.directory && e.path.split('/').at(-1) === 'SKILL.md')
  ...
  const files = entries.filter((e) => !e.directory && e.path.startsWith(prefix)).map((e) => {
    const compressed = data.subarray(e.start, e.start + e.compressed)
    const content = e.method === 0 ? Buffer.from(compressed) : inflateRawSync(compressed, { maxOutputLength: Math.max(1, e.original) })
