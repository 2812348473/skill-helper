...
export const LIMITS = Object.freeze({ file: 1024 * 1024, total: 8 * 1024 * 1024, archive: 16 * 1024 * 1024, files: 64, entries: 128 })
export const fail = (code, message = code) => Object.assign(new Error(message), { code })
export function segment(value) {
  if (typeof value !== 'string' || !/^(?!\.{1,2}$)[A-Za-z0-9._-]{1,128}$/.test(value) || value.endsWith('.') ||
    /^(con|prn|aux|nul|com[0-9]|lpt[0-9])(?:\.|$)/i.test(value)) throw fail('INVALID_NAME')
  return value
}
export function filePath(value) {
  if (typeof value !== 'string' || value.length > 640 || value.split('/').length > 5) throw fail('INVALID_FILE_PATH')
...
export function checkedFiles(input) {
  ...
  if (directories.size + seen.size > LIMITS.entries || [...directories].some((dir) => seen.has(dir))) throw fail('FILE_DIRECTORY_CONFLICT')
  const document = result.find((file) => file.path === 'SKILL.md')
  if (!document || document.content.length > 256 * 1024 || !/^---\r?\n/.test(document.content.toString('utf8'))) throw fail('SKILL_DOCUMENT_REQUIRED')
  return result
}
export function localPath(input, cwd) {
  if (typeof input !== 'string' || !input || [...input].some((c) => c.charCodeAt(0) < 32)) throw fail('INVALID_LOCAL_PATH')
  return isAbsolute(input) ? input : resolve(cwd, input)
}
export function requireInstall(ctx) {
  if (!ctx.skillInstall?.request) throw fail('AGH_UPGRADE_REQUIRED', '请更新 AGH：当前宿主没有受控 Skill 安装通道。')
...
export function httpsURL(value) {
  ...
  if (url.protocol !== 'https:' || url.username || url.password || url.hash || (url.port && url.port !== '443')) throw fail('PUBLIC_HTTPS_REQUIRED')
  return url
}
export async function publicText(ctx, value) {
  ...
  if (response.body.kind !== 'text') throw fail('DIRECT_TEXT_REQUIRED', '需要原始 Markdown 或 JSON 地址；网页、登录页和远程压缩包请先下载到本地。')
  const content = response.body.content
  if (Buffer.byteLength(content) > LIMITS.file * 2) throw fail('DOWNLOAD_TOO_LARGE')
  return content
}
export async function publicJSON(ctx, url) {
  try { return JSON.parse(await publicText(ctx, url)) } catch (error) {
