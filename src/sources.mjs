...
export const sourceAdapters = Object.freeze({
  ...
  async url(ctx, input) {
    ...
    if (/\.(zip|gz|tar|7z)(?:$|\/)/i.test(url.pathname)) throw fail('DOWNLOAD_ARCHIVE_LOCALLY', '远程压缩包请先下载到本地，再导入；不会绕过 AGH 公开网络保护。')
    const text = await publicText(ctx, url.href)
    if (/^---\r?\n/.test(text)) {
      if (!input.name) throw fail('SKILL_NAME_REQUIRED', '导入单个 Markdown 时请提供安装目录名 name。')
      return { name: input.name, files: checkedFiles([{ path: 'SKILL.md', content: text }]), message: '单文件导入不包含文中链接的附属文件。' }
    }
    let manifest
    try { manifest = JSON.parse(text) } catch { throw fail('DIRECT_SKILL_OR_MANIFEST_REQUIRED') }
    if (manifest?.version !== 1 || typeof manifest.name !== 'string' || !Array.isArray(manifest.files)) throw fail('INVALID_SKILL_MANIFEST')
